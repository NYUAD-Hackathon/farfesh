...
class FileDumperListener(StreamListener):
	...
	def __init__(self,filepath):
		...
		d=datetime.today()
		self.filename = "%i-%02d-%02d.ar.tweets"%(d.year,d.month,d.day)		
		self.fh = codecs.open(self.basePath + "/" + self.filename,"w","utf-8")#open for appending just in case
	...
	def on_data(self, data):
		...
		#self.fh.write(data)
		text = json.loads(data)["text"].rstrip(os.linesep)
		text=text+'\n'+'NEW_LINE'+'\n'
		self.fh.write(text)
		self.tweetCount+=1
		#print type(data)
	...
	def rotateFiles(self):
		d=datetime.today()
		filenow = "%i-%02d-%02d.ar.json"%(d.year,d.month,d.day)
...
if __name__ == '__main__':
	while True:
		try:
			...
			fhTerms = codecs.open(outputDir+"/FILTER.AR","r","utf-8")
			terms=[]
			for line in fhTerms:
